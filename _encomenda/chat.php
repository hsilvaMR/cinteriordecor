...
include('../admin/funcao/upload_img.php');
session_start();
...
if($LANG=='pt'){
	...
	if(!$assunto){$retorna['aviso'] = "Campo assunto vazio!"; $aviso=1;}
	else if(!$mensagem){$retorna['aviso'] = "Campo mensagem vazio!"; $aviso=1;}
}

if($LANG=='en'){

	if(!$assunto){$retorna['aviso'] = "Empty subject field!"; $aviso=1;}
	else if(!$mensagem){$retorna['aviso'] = "Empty message field!"; $aviso=1;}
}
...
mysqli_query($lnk,"INSERT INTO venda_conversa_msg (id_venda_conversa, mensagem, data) VALUES ('$id', '$mensagem', '$data')"); 
...
if($LANG=='pt'){
	$retorna['sucesso'] = "Obrigado por entrar em contacto connosco! Seremos breves na resposta."; $aviso = 1;
}

if($LANG=='en'){
	$retorna['sucesso'] = "Thank you for contacting us! We will be brief in response."; $aviso = 1;
}
