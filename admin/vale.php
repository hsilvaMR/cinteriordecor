...
<article>
	...
	if($existe){extract(mysqli_fetch_array(mysqli_query($lnk,"SELECT * FROM vales_desconto WHERE id='$id'"))); $id_url = urldecode($urlPartes[3]);}
	...
	<div class="conteudo">
		<div class="linha">
			<div class="titulo">
				<h1><? if($existe){echo "Editar";}else{echo "Novo";}?> Vale de Desconto<h1>
							...
							<div class="grupo">
								...
								<div class="grupoDir">
									<select id="history" class="seL" name="oferta" onchange="historyChanged(this);">
										...
										<option class="selS" value="cupao" <? if($tipo=='cupao'){echo "selected";}?>>cupão em €</option>
			                        ...
			                        <div id="select_cupao" style="margin:20px 0 10px 0;display:none; <? if($tipo=='cupao'){echo "display:block";}?>" >
			                        	
			                        	<div style="float:left;">
			                        		<label>cupão de (€)</label>
			                        		<input type="text" class="inP" name="desconto_cupao" value="<? if($valor_desconto != 0){ echo $valor_desconto; } ?>" placeholder='20'>
			                        	</div>

			                        	<div style="float:left;margin-left:20px;">
			                        		<label>acima de (€) - opcional</label>
			                        		<input type="text" class="inP" name="desconto_acima_cupao" value="<? if($valor_condicao != 0){ echo $valor_condicao; }?>" placeholder='60'>
			                        	</div>
			                        </div>
